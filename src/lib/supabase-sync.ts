...
async function syncTable<T extends { id?: number | string; isSynced?: boolean }>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    db.logSystemEvent(
      'WARN',
      `Supabase sync ${supabaseTable} attempt ${attempt}/${maxRetries} failed`,
      { error }
    );
  ...
  if (lastError) {
    db.logSystemEvent(
      'ERROR',
      `Supabase error syncing table ${supabaseTable} after ${maxRetries} attempts`,
      { error: lastError }
    );
...
export async function syncToSupabase(
  dbInstance: Dexie & { [key: string]: Dexie.Table<any, any> }
  ...
  try {
    for (const plan of syncPlan) {
      const { upserted, errors } = await syncTable(
        supabase,
        plan.dexieTable,
        plan.supabaseTable,
        plan.transform,
    ...
    return {
      success: true,
      message: `Sincronización completada. ${totalUpserted} registros actualizados en la nube.`,
    };
  } catch (e: any) {
