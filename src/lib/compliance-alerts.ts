...
export async function checkAndSendComplianceAlerts(daysAhead: number = 30): Promise<number> {
  try {
    ...
    await db.logSystemEvent('ERROR', 'Failed to check compliance alerts', { error: (error as Error).message });
    return 0;
  }
}
 ...
 */
export async function getComplianceAlerts(daysAhead: number = 30): Promise<Array<RegulationCompliance & { 
  userName: string;
  ...
  try {
    ...
    const today = new Date();
    const alerts: Array<RegulationCompliance & { 
      userName: string;
      regulationName: string;
      regulationCode: string;
      daysUntilExpiry: number;
      ...
      for (const compliance of complianceRecords) {
        ...
        if (daysUntilExpiry <= daysAhead) {
          ...
          const status: 'expired' | 'expiring' | 'compliant' = 
            ...
            daysUntilExpiry <= 30 ? 'expiring' : 'compliant';
