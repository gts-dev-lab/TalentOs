...
const courseFormSchema = z.object({
  ...
  instructor: z
    .string()
    .min(2, { message: 'El nombre del instructor debe tener al menos 2 caracteres.' }),
  duration: z.string().min(1, { message: 'La duración es obligatoria.' }),
  modality: z.enum(['Online', 'Presencial', 'Mixta'], {
    errorMap: () => ({ message: 'Debes seleccionar una modalidad.' }),
  }),
  capacity: z.coerce
    .number()
    .int()
...
function AssociateResourceDialog({
  ...
  useEffect(() => {
    const initialSelection = associatedResourceIds.reduce(
      (acc, id) => {
        acc[id] = true;
        return acc;
      },
            ...
            <div key={resource.id} className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={checked =>
                  setSelected(prev => ({ ...prev, [resource.id!]: !!checked }))
                }
              />
              <Label htmlFor={`res-${resource.id}`} className="font-normal">
                {resource.name}
              </Label>
            </div>
          ))}
...
export default function EditCoursePage() {
  ...
  const associatedResourceIds = useLiveQuery(
    () => db.getAssociatedResourceIdsForCourse(courseId),
    [courseId],
    []
  );
  ...
  const handleSaveModule = (moduleData: Omit<Module, 'id'>) => {
    if (currentModule) {
      setModules(
        modules.map(m => (m.id === currentModule.id ? { ...currentModule, ...moduleData } : m))
      );
    } else {
    ...
    <div className="space-y-8">
        ...
        <div className="xl:col-span-2 space-y-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Aquí puedes modificar los detalles generales de la formación.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Dejar en blanco o en 0 para plazas ilimitadas.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
              ...
              <div className="space-y-2">
                {modules.length > 0 ? (
                  modules.map(module => (
                    <div
                      key={module.id}
                      className="flex items-center justify-between p-2 pr-1 border rounded-lg bg-muted/50 text-sm"
                    >
                      <p className="font-medium truncate flex-grow" title={module.title}>
                        {module.title}
                      </p>
                ...
                {associatedResources && associatedResources.length > 0 ? (
                  associatedResources.map(resource => (
                    <div
                      ...
                      className="flex items-center justify-between p-2 pr-1 border rounded-lg bg-muted/50 text-sm"
                    >
                      <p className="font-medium truncate flex-grow" title={resource.name}>
                        {resource.name}
                      </p>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-7 w-7 text-destructive hover:text-destructive"
                        onClick={() => handleDissociateResource(resource.id!)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
      ...
      <div className="flex justify-end pt-4">
        <Button
          ...
          size="lg"
          disabled={isSubmitting || (!isDirty && !modulesDirty)}
        >
