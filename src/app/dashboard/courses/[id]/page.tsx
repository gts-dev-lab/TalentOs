    ...
    <Card className="shadow-lg">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <CalendarDays /> Próximas Sesiones en Directo
        </CardTitle>
      ...
      <CardContent className="space-y-4">
        {upcomingEvents.map(event => (
          <div
            key={event.id}
            className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 p-3 border rounded-lg bg-muted/50"
          >
            <div>
...
export default function CourseDetailPage() {
  ...
  const [isDownloading, setIsDownloading] = useState<null | 'Clásico' | 'Moderno' | 'Profesional'>(
    null
  );
  ...
  const enrollmentCount = useLiveQuery(() => db.getApprovedEnrollmentCount(courseId), [courseId]);
  ...
  const completedModules = useMemo(
    () => new Set(userProgress?.completedModules || []),
    [userProgress]
  );
  ...
  const canFullyManage =
    user && (user.role === 'Administrador General' || user.role === 'Jefe de Formación');
    ...
    <div className="space-y-8">
      ...
      {canManageCourse && course.status === 'draft' && (
        <Alert variant="destructive" className="bg-yellow-50 border-yellow-300 text-yellow-800">
          ...
          <AlertDescription>
            Este curso es un borrador y no es visible para los usuarios normales. Publícalo para que
            esté disponible en el catálogo.
          </AlertDescription>
        </Alert>
      )}
            ...
            {course.capacity !== undefined && enrollmentCount !== undefined && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {enrollmentCount} / {course.capacity} plazas
                  </span>
                </div>
              </>
            )}
                                ...
                                <div className="pl-10">
                                  ...
                                  {summaryText ? (
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        handleGenerateSummary(module.id, module.content)
                                      }
                                      disabled={isLoadingSummary || !isEnrolled}
                                      title={
                                        !isEnrolled ? 'Debes estar inscrito para usar la IA' : ''
                                      }
                                    >
                                      {isLoadingSummary ? (
              ...
              {aiConfig?.enabledFeatures.questionGeneration && (
                <TabsContent value="test" className="mt-4">
                  <TestGenerator
                    ...
                    courseContent={course.longDescription}
                  ...
                  <Card className="shadow-lg">
                    <CardContent className="p-8 text-center">
                      ...
                      <p className="mt-1 text-muted-foreground">
                        Debes estar inscrito en este curso para ver y participar en las discusiones.
                      </p>
                      {!isEnrolled && (
            ...
            <CardFooter>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button className="w-full" disabled={progressPercentage < 100 || !!isDownloading}>
                    {isDownloading ? (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    ) : (
                ...
                >
                  <DropdownMenuItem
                    onSelect={() => handleDownloadCertificate('Clásico')}
                    disabled={isDownloading === 'Clásico'}
                  >
                    {isDownloading === 'Clásico' && (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    )}
                    Diseño Clásico{' '}
                    {aiConfig?.defaultCertificateTemplate === 'Clásico' && (
                      <span className="text-xs text-muted-foreground ml-2">(Por defecto)</span>
                    )}
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onSelect={() => handleDownloadCertificate('Moderno')}
                    disabled={isDownloading === 'Moderno'}
                  >
                    {isDownloading === 'Moderno' && (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    )}
                    Diseño Moderno{' '}
                    {aiConfig?.defaultCertificateTemplate === 'Moderno' && (
                      <span className="text-xs text-muted-foreground ml-2">(Por defecto)</span>
                    )}
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onSelect={() => handleDownloadCertificate('Profesional')}
                    disabled={isDownloading === 'Profesional'}
                  >
                    {isDownloading === 'Profesional' && (
                      <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    )}
                    Diseño Profesional{' '}
                    {aiConfig?.defaultCertificateTemplate === 'Profesional' && (
                      <span className="text-xs text-muted-foreground ml-2">(Por defecto)</span>
                    )}
                  </DropdownMenuItem>
                </DropdownMenuContent>
