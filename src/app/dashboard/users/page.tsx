...
export default function UsersPage() {
  ...
  const { pendingUsers, filteredActiveUsers, paginatedUsers, totalPages } = useMemo(() => {
    if (!users)
      return { pendingUsers: [], filteredActiveUsers: [], paginatedUsers: [], totalPages: 0 };
    ...
    const filteredActive = active.filter(
      u => roleFilters[u.role] && departmentFilters[u.department]
    );

    const totalPages = Math.ceil(filteredActive.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const paginatedUsers = filteredActive.slice(startIndex, endIndex);
  ...
  const canManageUsers =
    ['Gestor de RRHH', 'Jefe de Formación', 'Administrador General'].includes(authUser.role) ||
  ...
  return (
    <div className="space-y-8">
      ...
      {pendingUsers.length > 0 && (
        <Card className="border-amber-500/50 bg-amber-500/5 shadow-frappe">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="mt-2">
                  Estos usuarios se han registrado con roles que requieren aprobación manual. Revisa
                  y aprueba o rechaza cada solicitud.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="border rounded-frappe bg-background shadow-frappe-sm">
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableRow key={u.id} className="hover:bg-muted/50">
                      ...
                      <TableCell className="text-right">
                        <div className="flex gap-2 justify-end">
                          <Button
                            size="sm"
                            onClick={() => setAction({ type: 'approve', user: u })}
                          ...
                          <Button
                            size="sm"
                            variant="destructive"
                            onClick={() => setAction({ type: 'reject', user: u })}
                          >
                    ...
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onSelect={() => setAction({ type: 'delete', user: u })}
