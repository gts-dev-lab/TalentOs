            ...
            >
              ...
              <div className="flex-grow">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  {isClient
                    ? formatDistanceToNow(new Date(announcement.timestamp), {
                        addSuffix: true,
                        locale: es,
                      })
                    : '...'}
              ...
              <Badge
                variant={announcement.type === 'Urgente' ? 'destructive' : 'secondary'}
                className="h-fit"
              >
                {announcement.type}
              </Badge>
...
function MandatoryCoursesPanel({ user }: { user: User }) {
  const incompleteCourses = useLiveQuery(
    () => db.getIncompleteMandatoryCoursesForUser(user),
    [user.id],
    []
  );
  ...
  return (
    <Card className="shadow-lg col-span-1 lg:col-span-3 border-amber-500/50 bg-amber-500/5">
      <CardHeader>
        ...
        <CardDescription>
          Tienes los siguientes cursos obligatorios por completar para cumplir con los requisitos de
          tu rol.
        </CardDescription>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card className="shadow-lg col-span-1 lg:col-span-1">
      <CardHeader>
        ...
        <CardDescription>
          Sugerencias de la IA basadas en tu perfil y formación externa.
        </CardDescription>
      </CardHeader>
...
function StudentDashboardView({ user }: { user: User }) {
  ...
  const enrolledCourses = useLiveQuery(
    () => (user ? db.getEnrolledCoursesForUser(user.id) : []),
    [user?.id]
  );
  const allProgress = useLiveQuery(
    () => (user ? db.getUserProgressForUser(user.id) : []),
    [user?.id]
  );
  const allCourses = useLiveQuery(() => db.getAllCourses(), []);
  ...
  const isManager = ['Gestor de RRHH', 'Jefe de Formación', 'Administrador General'].includes(
    user.role
  );
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        {isManager && (
          <StatCard
            title="Coste Total"
            value={`${totalCost.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}`}
            icon={Wallet}
            description="Presupuesto: 25,000€"
          />
        )}
      </div>
